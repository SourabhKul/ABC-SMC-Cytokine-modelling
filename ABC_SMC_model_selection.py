...
def first_iteration(N,ep):
   ...
   while number < N:
      ...
      for stim in range(2): #Loop for the two cytokine models.
         ...
         if stim == 0:      
            ...
            if model == 0: #Hypotheis 1
               beta = pow(10,np.random.uniform(-5,-1))
               gamma = 0
            else: #Hypothesis 2
               beta = 0
               gamma = pow(10,np.random.uniform(-5,-1))
            if model == 0:
      ...
      #and add one to the parameter set counter. Keep a list of the model which was accepted.
      if dist2 < epsilon:
         number+=1
   ...
   weights_hyp2 = np.empty((0,1))
   for i in range(len(accepted_params_hyp2)):
      weights_hyp2 = np.vstack((weights_hyp2,1/len(accepted_params_hyp2)))      
   ...
   return [np.hstack((np.reshape(results_hyp1,(len(accepted_params_hyp1),1)),accepted_params_hyp1,weights_hyp1)),
           np.hstack((np.reshape(results_hyp2,(len(accepted_params_hyp2),1)),accepted_params_hyp2,weights_hyp2)),truns,np.stack((accepted_models))]
