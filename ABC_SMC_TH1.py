...
def first_iteration(N,ep):
   ...
   while number < N: #Run the loop until N parameter sets are accepted.
      ...
      setx = np.random.randint(0,len(RPE_posteriors)) #Randomly select an RPE1 parameter set.
...
def other_iterations(N,it):
   ...
   while number < N: #Run the loop until N parameter sets are accepted.
      ...
      check = 0
      while check < 1:
         choice = np.random.choice(p_list,1,p=ABC_runs[it][:,26]) #Choose a random parameter set from previous iterations posteriors.
